...
##### modifying by WT.Jin 02/28/2020  ---  Start
class CourseDeleteView(UserPassesTestMixin, DeleteView):
    def test_func(self):
        return self.request.user.account_type == "teacher" or self.request.user.account_type == "school"

    model = SchoolCourse
    slug_field = "slug"
    template_name = 'delete.html'
    success_url = reverse_lazy('school_courses:manage_course_list')

# class SchoolCourseDeleteView(UserPassesTestMixin, DeleteView):
#     def test_func(self):
...
#     slug_field = "slug"
#     template_name = 'delete.html'
#     success_url = reverse_lazy('schools:courses:course_list')
...
##### modifying by WT.Jin 02/28/2020  ---  end
...
##### modifying by WT.Jin 02/28/2020  ---  Start
    ...
    def dispatch(self, request, course_id, module_slug=None):
        self.course = self.get_course(request, course_id)

        if module_slug:
            self.module = self.get_module(module_slug, course_id)
        return super(ModuleCreateUpdateView, self).dispatch(request, course_id, module_slug)
    ...
    def post(self, request, course_id, *args, **kwargs):
        if self.module:
            ...
            if self.form.is_valid():
                self.form.save()
                return redirect("school_courses:module_list", course_id)
        else:
            ...
            if self.form.is_valid():
                ...
                return redirect("school_courses:module_list", course_id)  #####  modifying by Wt.Jin 02/28/2020   return redirect('schools:teachers:manage....')
...
class ContentCreateUpdateView(UserPassesTestMixin, LoginRequiredMixin, TemplateResponseMixin, View):
    ...
    def get_model(self, model_name):
        if model_name in ['stext', 'svideo', 'simage', 'sfile']:
            return apps.get_model(app_label='school_courses', model_name=model_name)
        return None
        ...
        if form.is_valid():
            ...
            if not id:
                ...
                SchoolContent.objects.create(module=self.module,
                                             item=obj)
            return redirect('schools:teachers:courses:module_content_list', self.module.slug)
        return self.render_to_response({'form': form,
                                        'object': self.obj,
                                        'model': model_name})
...
class ModuleListView(UserPassesTestMixin, LoginRequiredMixin, TemplateResponseMixin, View):
    ...
    def get(self, request, course_slug):
        if request.user.account_type == 'teacher':
            ...
            return HttpResponse("sjdfsfhskdhf")

        return self.render_to_response({'module': self.module,
                                        'course': self.course})
########  Modifying by WT.Jin 02/28/2020 --- converting the slug to id
...
class CourseListView(UserPassesTestMixin, LoginRequiredMixin, TemplateResponseMixin, View):
    ...
    def get(self, request, subject=None):
        if self.request.user.account_type == 'school':
            ...
            # return HttpResponse("sdhfsldjhflskdjflk")
