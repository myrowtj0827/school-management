...
from schools.models import SchoolStudent, Teacher
...
class Quiz(models.Model):
    ...
    course = models.ForeignKey(
        SchoolCourse, null=True, blank=True,
...
class Progress(models.Model):
    ...
    """
    student = models.OneToOneField(SchoolStudent, verbose_name=_("Student"), on_delete=models.CASCADE)
    ...
    # @property
    def list_all_cou_scores(self, enrolled_student):
        """
        ...
        for cou in SchoolCourse.objects.filter(students__in=[enrolled_student]):
            to_find = re.escape(cou.title) + r",(\d+),(\d+),"
            ...
            if match:
                ...
                output[cou.title] = [score, possible, percent]

            else:  # if category has not been added yet, add it.
                self.score += cou.title + ",0,0,"
                output[cou.title] = [0, 0]
    ...
    def update_score(self, question, score_to_add=0, possible_to_add=0):
        ...
        """
        course_test = SchoolCourse.objects.filter(title=question.course) \
            .exists()
    ...
    def show_exams(self):
        ...
        """
        return Sitting.objects.filter(user=self.student, complete=True)

    def __str__(self):
        return self.student.user.username + ' - ' + self.score
...
class Sitting(models.Model):
    ...
    user = models.ForeignKey(SchoolStudent, verbose_name=_("Student"), on_delete=models.CASCADE)
    ...
    def get_first_question(self):
        ...
        return MCQQuestion.objects.get(id=question_id)
